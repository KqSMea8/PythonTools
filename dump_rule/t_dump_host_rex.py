...
def load_rule():
    ...
    sql = "select a.usertags,a.host_set_object,a.plan_id from adp_group_info as a,adp_plan_info as b where a.plan_id=b.plan_id and a.enable =1 and b.enable=1 and a.group_id=196;"
    cursor = conn.cursor()
    cursor.execute(sql)
    res = cursor.fetchall()
    if res:
