...
def parse_href(href):
    try:
        timeout = (10, 30)
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64; rv:44.0) Gecko/20100101 Firefox/44.0'}
        r = requests.get(url=href, headers = headers, timeout=timeout)
        return r.url
    except requests.ConnectTimeout:
        print("connect url {0} time out".format(href))
        return ''
    except requests.ReadTimeout:
        print("read url {0} time out".format(href))
        return ''
    except:
        return ''
...
def get_baidu_results_pc_per_page( keyword, result_url_set, page_index=0 ):
    ...
    try:
        r2 = requests.get(url=url, params=params, headers = headers, timeout=timeout)
    except requests.ConnectTimeout:
        print("connect url {0} time out".format(url))
        return 1
    except requests.ReadTimeout:
        print("read url {0} time out".format(url))
        return 1
    # print r2.text
