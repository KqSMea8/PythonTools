...
def chinese_train(cl, dir_name):
    current_path = os.path.split(os.path.realpath(__file__))[0] + '/'
    for file in glob.glob(current_path + os.sep + dir_name + os.sep + EXCEL_PREFIX):
        print(file)
...
def connect_mysql(host="127.0.0.1", port=3306, user="root", password="", database="test"):
    conn = MySQLdb.connect(host=host, port=port, user=user, passwd=password, db=database, charset='utf8')

    return conn


def mysql_train(host="127.0.0.1", port=3306, user="root", password="", database="test"):
    conn = connect_mysql(host=host, port=port, user=user, passwd=password, db=database, charset='utf8')
    cursor = conn.cursor()
