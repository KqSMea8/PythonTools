...
"""
这个脚本用于解析bidder日志，生成各个广告在一天的展现和点击次数
...
def generate_click_map_file_thread(files, id_filter):
...
def main():
    ...
    if (6 != len(sys.argv)):
        ...
        log_dir = raw_input("Please enter the directory name of the  log file:\n")
    ...
    print_cur_time()
    log_files = get_file_list(log_dir)
    print("file number of bidder log is %d" % len(log_files))
    print_cur_time("finish get_file_list is ")

    click_dict = generate_click_map_file_thread(log_files, "rtb_click")
    print_cur_time("finish generate_click_map is ")
    print(len(click_dict.keys()))
    print_click_dict(click_dict)

    click_threads = []
    thread_num = len(log_files) / CLICK_FILES_PER_THREAD + 1
    print("click threads num: %d" % thread_num)
    for i in range(thread_num - 1):
        t = MyThread(generate_click_map_file_thread,
                     (log_files[i * CLICK_FILES_PER_THREAD: (i + 1) * CLICK_FILES_PER_THREAD], zj_aid, "rtb_click"),
                     generate_click_map_file_thread.__name__ + "_" + str(i))
        click_threads.append(t)
    t = MyThread(generate_click_map_file_thread,
                 (log_files[thread_num - 1 * CLICK_FILES_PER_THREAD: len(zj_files)], zj_aid, "rtb_click"),
    ...
    for i in range(thread_num - 1):
        t = MyThread(parse_create_pushid_site_file_thread,
                     (zj_click_dict, log_files[i * SHOW_FILES_PER_THREAD: (i + 1) * SHOW_FILES_PER_THREAD], zj_aid, "rtb_creative"),
                     parse_create_pushid_site_file_thread.__name__ + "_" + str(i))
        show_threads.append(t)
    t = MyThread(parse_create_pushid_site_file_thread,
                 (zj_click_dict, log_files[thread_num - 1 * SHOW_FILES_PER_THREAD: len(zj_files)], zj_aid, "rtb_creative"),
