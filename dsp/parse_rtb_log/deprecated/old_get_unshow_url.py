    ...
    if (not os.path.isfile(file_name)):
        ...
        with open(file_name) as fd:
            for line in fd:
                ...
                if (0 == cmp(strs[0], "rtb_creative") and True !=
                    is_key_in_list(strs[7], show_pushid_list) and 0 ==
                    cmp(strs[6], ad_id)):
...
def generate_show_pushid(ad_id, show_pushid_list, log_dir_name):
    ...
    if os.path.isdir(log_dir_name):
        for parent, dirnames, filenames in os.walk(
                log_dir_name):  # three parameters return 1.parent directory, 2.directorys, 3.files
            # for dirname in dirnames:                       #display directory information
...
def print_unshowurl_dict(unshow_url_dict):
    for key in unshow_url_dict.keys():
        time_url_tuple = unshow_url_dict[key]
        print('{0},{1},{2}'.format(time_url_tuple[0], time_url_tuple[1],
                                   time_url_tuple[2]))
...
out_file = out_dir + "unshow_url.txt"
